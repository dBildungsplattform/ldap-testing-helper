    ...
    for lehrer_admin in potantial_mergable_lehrer_admins:
        for lehrer in all_lehrer:
            if lehrer_admin['given_name'] == lehrer['username'] and lehrer_admin['schule'] == lehrer['schule']:
                ...
                log(f"Found One Mergable: {lehrer_admin['given_name']} == {lehrer['username']} AND {lehrer_admin['schule']} == {lehrer['schule']}")
                break
            
    log(f"Number of Total Persons: {total_persons}")
    log(f"Number of Potential Mergable Admin Persons {len(potantial_mergable_lehrer_admins)}")
    log(f"Number of Lehrer Persons {len(all_lehrer)}")
    log(f"Total Number of Mergable: {total_number_of_mergable}")
